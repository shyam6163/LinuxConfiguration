...
- The web application is my [Item Catalog project](https://github.com/shyam6163/Item-Catalog.git) created earlier in this Nanodegree program.
...
1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 18.205.194.121
  - we can download a .pem file and connect with following command
    ```
    ssh -i catalog.pem ubuntu@18.205.194.121
    ``` 
...
7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i catalog.pem -p 2200 grader@18.205.194.121`.
...
`sudo git clone https://github.com/shyam6163/Item-Catalog`.
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add  http://18.205.194.121.xip.io and 
http://ec2-18-205-194-121.compute-1.amazonaws.com/ as authorized JavaScript 
origins.
- Add http://18.205.194.121.xip.io/login,http://18.205.194.121.xip.io/gconnect,http://18.205.194.121.xip.io/callback
...
15: Set up and enable a virtual host
  ...
  ```
  <VirtualHost *:80>
      ServerName 18.205.194.121.xip.io
      ServerAlias ec2-18.205.194.121.compute-1.amazonaws.com
      ServerAdmin ubuntu@18.205.194.121
